...
async function fetchKalshiPublicMarkets() {
  ...
  // Multi-endpoint parallel fetch against Kalshi's verified elections gateway
  const endpoints = [
  ...
  for (const res of results) {
    ...
    marketBatch.forEach(m => {
      ...
      // Exclude unsolvable Millennium Prize problems
      if (/conjecture|hypothesis|swinnerton|millennium prize|riemann|p versus np|hodge/i.test(titleStr)) {
        return;
      }

      // Allow contracts up through the 2028 election cycle (currentYear + 4), only exclude 2050/2070/2099 placeholders
      const expTime = m.expiration_time || m.close_time || "";
      if (expTime && new Date(expTime).getUTCFullYear() > currentYear + 4) return;
